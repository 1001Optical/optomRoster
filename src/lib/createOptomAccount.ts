...
export const createOptomAccount = async (name: string, email: string) => {
    ...
    try {
        ...
        if (nameParts.length < 2) {
            throw new Error(`Invalid name format: ${name}. Expected "FirstName LastName"`);
        }
        
        const [givenName, surname] = nameParts;
        console.log(`Parsed name - Given: ${givenName}, Surname: ${surname}`);
